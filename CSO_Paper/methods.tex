...
\subsection{Our Proposed Pipeline: The Joint Optimization via Competitive Swarm Optimizer}
\label{sec:3.6}

The contribution of this study is not a new metaheuristic operator but a
new object for an existing one to search over. As reviewed in
Section~\ref{sec:2.3}, published metaheuristic pipelines for PD voice
screening almost universally treat the transfer function, the feature
subset, and the classifier configuration as three separate decisions, of
which at most two are ever optimized together and the third is fixed by
hand in advance. This section describes the pipeline that instead
searches all three jointly, within the single $5+D+5$-dimensional
encoding of Section~\ref{sec:3.3} and the fitness of Section~\ref{sec:3.5},
using the Competitive Swarm Optimizer (CSO) \cite{Cheng:2015} as the
search engine that explores this space generation by generation. CSO's
own pairwise-competition update rule is adopted in its original,
published form -- no new operator is layered on top of it -- but, as
detailed below, every one of its competitions now also implicitly
resolves which of five binarization schemes governs the losing particle
going forward, not only which feature-and-classifier configuration it
moves toward.

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/CSO.pdf}
  \caption{The CSO per-generation flow \cite{Cheng:2015}, adopted by this
    pipeline in its original form: the population is randomly paired into
    one-on-one competitions; each pair's winner passes to the next
    generation unchanged (learning-free), while its loser updates toward
    the winner (learning). Figure~\ref{fig:tf-selector} details what
    "learning" means once the transfer-function-selector segment is part
    of the loser's own position.}
  \label{fig:cso}
\end{figure}

\textbf{Search engine: CSO.} The Competitive Swarm Optimizer (CSO)
...
updates. This property is the reason CSO is adopted here rather than,
e.g., a PSO variant already reproduced among the baselines
(Section~\ref{sec:3.7}): the joint encoding of Section~\ref{sec:3.3} is 5
dimensions larger than a feature-and-classifier-only encoding of the same
dataset, and resistance to premature convergence is correspondingly more
valuable, not merely a large-scale-benchmark curiosity
(Section~\ref{sec:2.3}). Concretely, each generation, the population of
$N$ particles (an even number) is randomly paired into $N/2$ one-on-one
...
study ($N=30 \le 100$) -- so at this scale the update reduces to
winner-attraction and inertia only, with no mean-position pull. No term
above is specific to feature selection, classifier choice, or
transfer-function selection: CSO updates whichever position vector it is
given, which is precisely what lets Section~\ref{sec:3.3}'s larger,
jointly-encoded position slot into an otherwise-unmodified update rule.
...
exactly as its feature-mask and classifier-mask segments can change which
features and classifiers it uses. Once a loser's new position is
computed, it is decoded in two steps (Figure~\ref{fig:tf-selector}):
first, an argmax over its own leading 5-dim selector segment picks one of
the 5 candidate transfer functions of Section~\ref{sec:3.4}; second, that
candidate binarizes the particle's trailing $D+5$ dimensions into the
feature mask and classifier mask used to evaluate its fitness
(Section~\ref{sec:3.5}). Winners are unaffected, since they are carried
over completely unchanged, selector segment included. This decode step,
not CSO's update rule, is where this study's contribution is realized: it
is what turns a population of otherwise-ordinary CSO particles into a
population that is simultaneously searching feature subsets, classifier
configurations, and its own binarization scheme, one joint decision per
particle per generation.
...
Taken together, Sections~\ref{sec:3.3}--\ref{sec:3.6} describe one
pipeline, not four independent pieces: the encoding determines what a
particle's position means (transfer-function choice, feature subset, and
classifier configuration, all at once); the candidate pool of
Section~\ref{sec:3.4} determines what a transfer-function choice can
resolve to; the fitness of Section~\ref{sec:3.5} determines how any one
decoded solution is scored; and CSO, described above, determines how the
population moves through that space generation by generation. Reusing
CSO's own competitive update rule unchanged is a deliberate design
choice made in service of this pipeline, so that any performance
difference against the reproduced baselines of Section~\ref{sec:3.7} can
be attributed to the joint encoding and its decode step rather than to a
new, unproven movement rule -- it is not evidence that the proposed
pipeline reduces to CSO itself: a plain CSO run over the baselines' own
$D+5$ encoding would have no transfer-function-selector segment to search
over at all, and so could not carry out the joint optimization this
