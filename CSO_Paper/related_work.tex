...
grounding is revisited in Section~\ref{sec:5} when interpreting
...
\cite{Appati:2026}. As discussed in Section~\ref{sec:1}, the
...
transfer-function-, feature-, and classifier encoding proposed in this
study (Section~\ref{sec:3}) is considerably larger than a
...
large-scale-benchmark curiosity. Section~\ref{sec:3} describes this
encoding in full: rather than adding new search strategies on top of
plain CSO, this study leaves CSO itself unmodified and instead extends
what each particle's position encodes, so the choice of transfer
function -- ordinarily fixed for an entire study, as surveyed above -- is
searched per particle alongside the feature subset and classifier
selection.
